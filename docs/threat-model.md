...
| Output exhaustion | stdout/stderr and artifact text are truncated to configured byte limits. Search has result and output limits. | `src/workbench_mcp/security/limits.py`, `src/workbench_mcp/services/process_runner.py`, `src/workbench_mcp/services/search.py`, `src/workbench_mcp/services/artifact_service.py` | `tests/unit/test_process_runner.py`, `tests/unit/test_search_service.py` | Very large directory trees can still cost traversal time until limits are reached. |
...
| Malicious workspaces | Workspace reads/searches avoid traversal and common generated directories; command CWD remains inside workspace. | `src/workbench_mcp/services/filesystem.py`, `src/workbench_mcp/services/search.py`, `src/workbench_mcp/services/process_runner.py` | `tests/security/test_path_security.py`, `tests/unit/test_filesystem_services.py` | Running allowlisted commands in a malicious workspace can execute untrusted code. |
...
| Git mutation | Only read-only Git status and diff-stat commands are implemented and no destructive Git tools are registered. | `src/workbench_mcp/services/git_service.py`, `src/workbench_mcp/tools/git_tools.py` | `tests/unit/test_git_service.py`, `tests/unit/test_mcp_server.py` | Git hooks are not invoked by current read-only commands, but future Git operations must be reviewed carefully. |
